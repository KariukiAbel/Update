            ...
            if (response.status == "success"){
                ...
                for (var i in result){
                    if(result[i]["images"] != ""){
                        ...
                        var src = "https://via.placeholder.com/70";
                    }
                    output += `
                    <div class="card">
                    <div class="card-body">
                    <h5 class="card-title">${result[i]["product_name"]}</h5>
                    <div><img class="card-img-top hover-shadow" onclick="openlightbox()" src='${src}'></div>
                    <div><p>${result[i]["description"]}</p></div>
                    </div>
                    <div class="card-footer">
                    <big class="text-muted">Ksh ${result[i]["selling_price"]}</big>
                    <button class="btn-outline-primary cart" id="${result[i]['id']}" style="float: right;"><i class="fas fa-shopping-cart"></i>Buy</button>
                    </div>
                    </div>
                    `;}
                output +=``
